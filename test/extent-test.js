...
tape("quadtree.extent(extent) extends the extent", function(test) {
  test.deepEqual(d3_quadtree.quadtree().extent([[0, 1], [2, 6]]).extent(), [[-1.5, 1], [3.5, 6]]);
  test.end();
});
...
tape("quadtree.extent() can be inferred by quadtree.add", function(test) {
  var q = d3_quadtree.quadtree();
  q.add(0, 0);
  test.deepEqual(q.extent(), [[0, 0], [0, 0]]);
  q.add(2, 4);
  test.deepEqual(q.extent(), [[-1, 0], [3, 4]]);
  test.end();
});
...
tape("quadtree.extent(extent) flips inverted extents", function(test) {
  test.deepEqual(d3_quadtree.quadtree().extent([[1, 1], [0, 0]]).extent(), [[0, 0], [1, 1]]);
  test.end();
});

tape("quadtree.extent(extent) tolerates partially-valid extents", function(test) {
  test.deepEqual(d3_quadtree.quadtree().extent([[NaN, 0], [1, 1]]).extent(), [[1, 1], [1, 1]]);
  test.deepEqual(d3_quadtree.quadtree().extent([[0, NaN], [1, 1]]).extent(), [[1, 1], [1, 1]]);
  test.deepEqual(d3_quadtree.quadtree().extent([[0, 0], [NaN, 1]]).extent(), [[0, 0], [0, 0]]);
  test.deepEqual(d3_quadtree.quadtree().extent([[0, 0], [1, NaN]]).extent(), [[0, 0], [0, 0]]);
  test.end();
});

tape("quadtree.extent(extent) allows trivial extents", function(test) {
  test.deepEqual(d3_quadtree.quadtree().extent([[0, 0], [0, 0]]).extent(), [[0, 0], [0, 0]]);
  test.deepEqual(d3_quadtree.quadtree().extent([[1, 1], [1, 1]]).extent(), [[1, 1], [1, 1]]);
  test.end();
});
