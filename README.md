...
Returns the current extent [[*x0*, *y0*], [*x1*, *y1*]] of this quadtree, where *x0* and *y0* are the inclusive lower bounds and *x1* and *y1* are the inclusive upper bounds. The extent may be initialized when the quadtree is [created](#quadtree), and is expanded [explicitly](#quadtree_cover) or implicitly as points are [added](#quadtree_add). The extent may be undefined, as when a new quadtree is created without specifying an extent.
